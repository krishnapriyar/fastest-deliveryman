...
public class SOCheckTodayOrder extends JFrame {
    ...
    public SOCheckTodayOrder() {
        ...
        JLabel[] jlblDeliveryTime = new JLabel[queueItem.size()];
        ...
        for (int i = 0; i <= queueItem.size(); i++) {
            ...
            jlblDeliveryTime[i] = new JLabel(String.valueOf(sc.getDeliveryTime()));
            ...
            itemListing[i].add(jlblDeliveryTime[i]);
    ...
    public void getData() {
        if (connection() == true) {
            try {
                String sql = "SELECT SUM(SCHEDULEORDERITEM.SCHEDULE_QUANTITY * ITEM.ITEMUNITPRICE) AS TotalPrice, SCHEDULEORDER.SCHEDULE_ORDERID, CUSTOMER.CUSTNAME,CUSTOMER.CUSTADDRESS,CUSTOMER.CUSTTELNO,SCHEDULEORDER.DISTANCE,SCHEDULEORDER.SCHEDULE_DELIVERYTIME FROM SCHEDULEORDER  LEFT JOIN SCHEDULEORDERITEM ON SCHEDULEORDER.SCHEDULE_ORDERID = SCHEDULEORDERITEM.SCHEDULE_ORDERID LEFT JOIN ITEM ON SCHEDULEORDERITEM.ITEMID = ITEM.ITEMID LEFT JOIN CUSTOMER ON SCHEDULEORDER.CUSTOMERID = CUSTOMER.CUSTID GROUP BY SCHEDULEORDER.SCHEDULE_ORDERID, CUSTOMER.CUSTNAME,CUSTOMER.CUSTADDRESS, CUSTOMER.CUSTTELNO, SCHEDULEORDER.DISTANCE, SCHEDULEORDER.SCHEDULE_DELIVERYTIME";
                stat = conn.createStatement();
                rs = stat.executeQuery(sql);

                while (rs.next()) {
                    queueItem.add(new ScheduleOrderClass(rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getDouble(1), rs.getInt(6), rs.getTime(7)));
                }
            } catch (Exception ex) {
                ex.printStackTrace();
            }
